...
export function AdaptiveSharePointTable({
      ...
      return {
        ...
        header: () => (
          ...
          >
            ...
            <div
              className={`flex items-center gap-1.5 font-semibold text-xs tracking-wider uppercase text-neutral-600 ${
            ...
            >
              ...
              <span className="text-[10px] font-mono font-normal lowercase px-1 py-0.2 rounded bg-neutral-200/70 text-neutral-500">
                {fieldType}
              </span>
            </div>
          </div>
  ...
  return (
    <div className="w-full space-y-4">
      {/* Table Top Toolbar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-xl border border-neutral-200/80 shadow-xs">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {totalAvailableColumns && totalAvailableColumns !== list.columns.length ? (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">
                <SlidersIcon className="w-3 h-3 text-blue-600" />
                Showing {list.columns.length} of {totalAvailableColumns} columns
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-mono text-neutral-500 bg-neutral-50 border border-neutral-200">
            ...
            {onEditConnection && (
              <button
                ...
                className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-700 hover:bg-blue-50 px-2 py-0.5 rounded-lg border border-transparent hover:border-blue-200 transition-colors cursor-pointer"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {availableLists && availableLists.length > 1 && onSelectListId && (
            <div className="relative">
              <select
                ...
                className="text-xs bg-neutral-50 hover:bg-neutral-100 focus:bg-white border border-neutral-300 focus:border-blue-500 rounded-lg px-2.5 py-1.5 font-medium text-neutral-800 outline-none transition-all cursor-pointer"
              >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 text-xs bg-neutral-50 hover:bg-neutral-100/80 focus:bg-white border border-neutral-300 focus:border-blue-500 rounded-lg outline-none w-44 sm:w-56 transition-all"
            />
            {globalFilter && (
              <button
                type="button"
          ...
          <div className="relative">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-neutral-700 bg-white hover:bg-neutral-50 border border-neutral-300 rounded-lg shadow-2xs transition-colors cursor-pointer"
            >
            ...
            {isColumnPickerOpen && (
              <div className="absolute right-0 mt-1 w-64 max-h-80 overflow-y-auto bg-white border border-neutral-200 rounded-lg shadow-lg z-50 p-2 text-xs divide-y divide-neutral-100">
                <div className="px-2 py-1 flex items-center justify-between font-medium text-neutral-500">
                    ...
                    .map((column) => {
                      return (
                        ...
                        <input
                          ...
                          className="rounded text-blue-600 border-neutral-300 focus:ring-blue-500"
                        />
                        <span className="truncate text-neutral-700">{column.id}</span>
                      </label>
                    );
                  })}
                </div>
      ...
      <div className="relative rounded-xl border border-neutral-200 bg-white shadow-xs overflow-hidden">
        ...
        <div className="relative overflow-x-auto overflow-y-auto max-h-[calc(100vh-190px)] min-h-[460px] select-text">
          <table className="w-full border-collapse text-left border-spacing-0">
            ...
            <thead className="sticky top-0 z-30 bg-neutral-50/95 backdrop-blur-xs border-b border-neutral-200">
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map((header, colIndex, arr) => {
                    ...
                    let stickyClass = 'relative bg-neutral-50/95';
                    if (isFirst) {
                      // Top-Left Sticky Corner
                      stickyClass =
                        'sticky left-0 z-40 bg-neutral-100 shadow-[2px_0_5px_-1px_rgba(0,0,0,0.08)] border-r border-neutral-200';
                    } else if (isLast) {
                      // Top-Right Sticky Corner
                      stickyClass =
                        'sticky right-0 z-40 bg-neutral-100 shadow-[-2px_0_5px_-1px_rgba(0,0,0,0.08)] border-l border-neutral-200';
                    }
                    ...
                    return (
                      <th
                        key={header.id}
                        scope="col"
                        className={`px-3.5 py-3 text-xs font-semibold select-none border-b border-neutral-200 ${stickyClass} ${minWidthClass}`}
                      >
                        {header.isPlaceholder ? null : (
                          <div
                            onClick={header.column.getToggleSortingHandler()}
                            className={`flex items-center justify-between gap-1.5 cursor-pointer group hover:text-neutral-900 transition-colors ${
                          ...
                          >
                            ...
                            {header.column.getCanSort() && (
                              <span className="text-neutral-400 group-hover:text-neutral-700 flex-shrink-0">
                                {{
                                  asc: <ArrowUp className="w-3.5 h-3.5 text-blue-600" />,
                                  desc: <ArrowDown className="w-3.5 h-3.5 text-blue-600" />,
            ...
            <tbody className="divide-y divide-neutral-100 bg-white">
              {table.getRowModel().rows.length === 0 ? (
                <tr>
                  <td colSpan={visibleColumnsCount} className="text-center py-12 text-neutral-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <Layers className="w-8 h-8 text-neutral-300" />
                      <p className="text-sm font-medium text-neutral-600">No matching SharePoint list items found</p>
                      <p className="text-xs text-neutral-400">Try clearing or modifying your filter query.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                table.getRowModel().rows.map((row, rowIndex) => {
                  ...
                  const rowBgClass = isEven ? 'bg-white' : 'bg-neutral-50';

                  return (
                    <tr
                      key={row.id}
                      className={`group hover:bg-blue-50 transition-colors ${rowBgClass}`}
                    >
                      {row.getVisibleCells().map((cell, colIndex, arr) => {
                        ...
                        if (isFirst) {
                          stickyClass = `sticky left-0 z-20 shadow-[2px_0_5px_-1px_rgba(0,0,0,0.06)] border-r border-neutral-200 ${
                            isEven ? 'bg-white' : 'bg-neutral-50'
                          } group-hover:bg-blue-50`;
                        } else if (isLast) {
                          stickyClass = `sticky right-0 z-20 shadow-[-2px_0_5px_-1px_rgba(0,0,0,0.06)] border-l border-neutral-200 ${
                            isEven ? 'bg-white' : 'bg-neutral-50'
                          } group-hover:bg-blue-50`;
                        }
                        ...
                        return (
                          <td
                            key={cell.id}
                            className={`px-3.5 py-3 text-xs text-neutral-700 align-middle ${stickyClass} ${minWidthClass}`}
                          >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 bg-neutral-50/90 border-t border-neutral-200 text-xs text-neutral-600">
          <div className="flex items-center gap-2">
            <span>
              Showing <strong className="text-neutral-900 font-semibold">{rowStart}</strong> to{' '}
              <strong className="text-neutral-900 font-semibold">{rowEnd}</strong> of{' '}
              <strong className="text-neutral-900 font-semibold">{totalRowsCount}</strong> items
            </span>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-1 rounded border border-neutral-300 bg-white hover:bg-neutral-100 disabled:opacity-40 disabled:pointer-events-none transition-colors"
              title="First page"
            >
            ...
            <button
              ...
              disabled={!table.getCanPreviousPage()}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded border border-neutral-300 bg-white hover:bg-neutral-100 disabled:opacity-40 disabled:pointer-events-none transition-colors"
            >
              <ChevronLeft className="w-3.5 h-3.5" />
              <span>Previous</span>
            </button>

            <span className="px-2 py-1 font-mono font-medium text-neutral-800 bg-white border border-neutral-200 rounded">
      ...
      {expandedRichText && (
        ...
        >
          <div
            className="relative bg-white rounded-xl shadow-2xl overflow-hidden max-w-xl w-full"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto text-sm text-neutral-800 leading-relaxed">
              <div
                className="space-y-3 prose prose-sm max-w-none text-neutral-800"
            ...
            <div className="px-5 py-3 bg-neutral-50 border-t border-neutral-200 flex justify-end">
              <button
                type="button"
                onClick={() => setExpandedRichText(null)}
                className="px-4 py-1.5 text-xs font-medium bg-neutral-200 hover:bg-neutral-300 text-neutral-800 rounded-lg transition-colors cursor-pointer"
              >
                Done
              </button>
            </div>
