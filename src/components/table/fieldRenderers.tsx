...
const BADGE_COLOR_PALETTES = [
  'bg-blue-50 text-blue-700 border-blue-200 ring-blue-500/10',
  ...
  'bg-purple-50 text-purple-700 border-purple-200 ring-purple-500/10',
  'bg-rose-50 text-rose-700 border-rose-200 ring-rose-500/10',
  'bg-teal-50 text-teal-700 border-teal-200 ring-teal-500/10',
  'bg-sky-50 text-sky-700 border-sky-200 ring-sky-500/10',
];

function getBadgeColor(text: string): string {
  if (!text) return 'bg-neutral-100 text-neutral-600 border-neutral-200';
  ...
  if (lower.includes('hold') || lower.includes('draft') || lower.includes('low')) {
    return 'bg-neutral-100 text-neutral-700 border-neutral-200 ring-1 ring-neutral-400/10';
  }
  let hash = 0;
...
export function SingleLineTextRenderer(props: SharePointFieldRendererProps) {
  ...
  return (
    <span className="truncate block font-normal text-neutral-800" title={text}>
      {text}
    </span>
  );
}
...
export function RichTextRenderer({ value, column, onExpandRichText }: SharePointFieldRendererProps) {
  ...
  return (
    <div className="flex flex-col gap-1 items-start max-w-sm">
      <div className="text-xs text-neutral-700 leading-relaxed overflow-hidden">
        {isExpandedLocal ? (
          <div
            className="prose prose-xs max-w-none text-neutral-800 bg-neutral-50 p-2 rounded border border-neutral-200"
            dangerouslySetInnerHTML={{ __html: rawHtml }}
          />
        ) : (
          <span className="line-clamp-2" title={plainText}>
            {plainText || <span className="italic text-neutral-400">Formatted content</span>}
          </span>
        )}
      </div>
      {isLong && (
      ...
      <div className="flex items-center gap-2 min-w-0" title={person.email || name}>
        {person.avatarUrl ? (
          <img
            ...
            className="w-6 h-6 rounded-full object-cover flex-shrink-0 border border-neutral-200"
          />
        ) : (
          <div className="w-6 h-6 rounded-full bg-neutral-200 text-neutral-700 flex items-center justify-center text-[10px] font-medium flex-shrink-0 border border-neutral-300">
            {initials || <User className="w-3 h-3" />}
          </div>
        )}
        <div className="truncate min-w-0">
          <span className="text-xs font-medium text-neutral-800 truncate block">{name}</span>
          {person.jobTitle && <span className="text-[10px] text-neutral-500 truncate block">{person.jobTitle}</span>}
        </div>
      </div>
    );
  }
      ...
      <div className="flex -space-x-1.5 overflow-hidden">
        {persons.slice(0, 3).map((p, idx) => {
          ...
          return p.avatarUrl ? (
            ...
            <div
              key={idx}
              className="inline-flex h-5 w-5 rounded-full ring-1 ring-white bg-neutral-300 items-center justify-center text-[9px] font-semibold text-neutral-700"
              title={p.displayName}
            >
              {initials}
            </div>
...
export function DateTimeRenderer({ value, column }: SharePointFieldRendererProps) {
  ...
  if (isNaN(dateObj.getTime())) {
    return <span className="text-xs text-neutral-600">{String(value)}</span>;
  }
  ...
  return (
    <div className="inline-flex items-center gap-1.5 text-xs text-neutral-700">
      ...
      {isDateTime && <span className="text-[11px] text-neutral-400 font-mono">{formattedTime}</span>}
    </div>
  );
}
...
export function NumberCurrencyRenderer({ value, column, fieldType }: SharePointFieldRendererProps) {
  ...
  if (isNaN(num)) {
    return <div className="text-right text-xs text-neutral-700">{String(value)}</div>;
  }
  ...
  if (fieldType === 'currency' || column.currency) {
    ...
    return (
      <div className="text-right font-mono text-xs font-medium text-neutral-800">
        <span className="text-neutral-500 mr-0.5">{symbol}</span>
        {formatted}
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      {isChecked ? (
        ...
        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-medium bg-neutral-50 text-neutral-500 border border-neutral-200">
  ...
  return (
    <div className="inline-flex items-center gap-2">
      <button
        ...
        className="group relative flex items-center gap-2 px-1.5 py-1 rounded-lg border border-neutral-200/90 bg-white hover:border-blue-400 hover:ring-2 hover:ring-blue-500/20 transition-all cursor-pointer shadow-2xs text-left"
        title="Hover to view full size image, click for full lightbox"
      >
        <div className="relative w-12 h-9 rounded-md overflow-hidden bg-neutral-100 border border-neutral-200 flex-shrink-0 flex items-center justify-center">
          <img
            src={url}
            alt={name}
            referrerPolicy="no-referrer"
        ...
        <div className="flex flex-col min-w-0 max-w-[130px]">
          <span className="text-[11px] font-medium text-neutral-800 truncate" title={name}>
            {name}
          </span>
          <span className="text-[10px] text-blue-600 font-medium group-hover:underline flex items-center gap-0.5">
            Full view
          </span>
        </div>
      </button>
      ...
      {isHovered && coords && createPortal(
        <div
          ...
          className="fixed z-[99999] pointer-events-none w-[380px] max-w-[94vw] bg-white rounded-xl shadow-2xl border border-neutral-200/90 overflow-hidden animate-in fade-in zoom-in-95 duration-150"
        >
          {/* Header Bar */}
          <div className="flex items-center justify-between px-3.5 py-2.5 bg-neutral-50 border-b border-neutral-200/80">
          ...
          {/* Footer Bar */}
          <div className="px-3.5 py-2 bg-neutral-50/70 flex items-center justify-between text-[10px] text-neutral-500 border-t border-neutral-100">
            <span className="font-mono text-[9px] text-neutral-400 truncate max-w-[220px]">
...
export function HyperlinkRenderer(props: SharePointFieldRendererProps) {
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 hover:underline max-w-xs truncate"
      ...
      {items.map((item, idx) => {
        ...
        return (
          <a
            ...
            className="inline-flex items-center gap-1 px-2 py-1 bg-neutral-100 hover:bg-neutral-200 border border-neutral-200 rounded text-xs text-neutral-700 transition-colors"
            title={name}
          >
    ...
    <div className="flex flex-wrap gap-1 items-center">
      {lookups.map((item, idx) => {
        ...
        return (
          <span
            key={idx}
            className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs bg-slate-100 text-slate-800 border border-slate-200"
            title={`Lookup: ${text}`}
          >
...
export function DefaultFallbackRenderer({ value }: SharePointFieldRendererProps) {
  ...
  if (typeof value === 'object') {
    return (
      <span className="text-xs text-neutral-600 font-mono truncate block max-w-xs" title={JSON.stringify(value)}>
        {JSON.stringify(value)}
      </span>
    );
  }
  return <span className="text-xs text-neutral-800 truncate block">{String(value)}</span>;
}
