...
export function DeleteRowConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs animate-in fade-in duration-150">
      <div
        className="bg-white rounded-xl shadow-2xl border border-neutral-200 w-full max-w-md overflow-hidden transform animate-in zoom-in-95 duration-150"
        role="dialog"
        aria-modal="true"
        aria-labelledby="delete-row-modal-title"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100 bg-rose-50/50">
          <div className="flex items-center gap-2.5 text-rose-700">
            <div className="p-2 bg-rose-100 text-rose-600 rounded-lg">
              <AlertTriangle className="w-5 h-5" />
            </div>
            <div>
              <h3 id="delete-row-modal-title" className="text-sm font-bold text-neutral-900">
                Confirm Row Deletion
              </h3>
              <p className="text-xs text-rose-700/80">Permanent action in demo dataset</p>
            </div>
          </div>
          <button
            type="button"
        ...
        {/* Footer Actions */}
        <div className="flex items-center justify-end gap-2.5 px-5 py-3.5 bg-neutral-50 border-t border-neutral-200">
          <button
            type="button"
            onClick={onClose}
